...
test('default', async t => {
	const c = new ClientAccountTxlist(new Address(address), '0', '999999', new Sort('desc'))
	const json = c.toJson()
	const network = new Network()
	const url = requestUrlBuilder(network, 'account', 'txlist', json)
	...
	await performRequest(network, 'account', 'txlist', json)
	scope.done()
	t.truthy(url)
})
