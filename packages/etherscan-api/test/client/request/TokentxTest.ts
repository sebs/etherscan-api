...
test('default', async t => {
	const c = new ClientAccountTokentx(new Address(address), '0', '999999', new Sort('desc'))
	const json = c.toJson()
	const network = new Network()
	const url = requestUrlBuilder(network, 'account', 'tokentx', json)
	...
	await performRequest(network, 'account', 'tokentx', json)
	scope.done()
	t.truthy(url)
})
