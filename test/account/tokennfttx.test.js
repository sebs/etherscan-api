...
describe('account.tokennfttx', function () {
  ...
  describe('with a contractaddress', function () {
    ...
    beforeEach(async function () {
      ...
      const res = await mocked.api.account.tokennfttx(ADDRESS, CONTRACT);
      result = res.result;
    });
    ...
    it('uses the tokennfttx action', function () {
      assert.equal(queryOf(transport).get('action'), 'tokennfttx');
    });
  ...
  describe('without a contractaddress', function () {
    ...
    beforeEach(async function () {
      const mocked = mockApi({ status: '1', result: 'ok' });
      transport = mocked.transport;
      const res = await mocked.api.account.tokennfttx(ADDRESS);
      result = res.result;
    });
