...
describe('transaction.getstatus', function () {
  let transport;
  let result;
  ...
  beforeEach(async function () {
    ...
    const res = await mocked.api.transaction.getstatus(TXHASH);
    result = res.result;
  });
  ...
  it('uses the getstatus action', function () {
    assert.equal(queryOf(transport).get('action'), 'getstatus');
  });
