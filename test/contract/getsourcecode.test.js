...
describe('contract.getsourcecode', function () {
  let transport;
  let result;
  ...
  beforeEach(async function () {
    ...
    const res = await mocked.api.contract.getsourcecode(ADDRESS);
    result = res.result;
  });
  ...
  it('uses the getsourcecode action', function () {
    assert.equal(queryOf(transport).get('action'), 'getsourcecode');
  });
