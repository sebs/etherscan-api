...
describe('proxy.eth_getTransactionReceipt', function () {
  let transport;
  let result;
  ...
  beforeEach(async function () {
    ...
    const res = await mocked.api.proxy.eth_getTransactionReceipt(TXHASH);
    result = res.result;
  });
  ...
  it('uses the eth_getTransactionReceipt action', function () {
    assert.equal(queryOf(transport).get('action'), 'eth_getTransactionReceipt');
  });
